...
export default function ExperienceTimeline() {
  return (
    <GlassCard id="experience" className="rounded-xl p-space-6">
      <h2 className="mb-space-6 font-display text-h2 text-text-primary">
        Experience
      </h2>
      <motion.ol
      ...
      >
        {experience.map((item) => (
          ...
          >
            ...
            <h3 className="mt-space-1 text-body-lg text-text-primary">
              {item.role}
            </h3>
            <p className="text-caption text-accent-primary">{item.company}</p>
            <p className="mt-space-2 text-body text-text-secondary">
              {item.summary}
            </p>
          </motion.li>
        ))}
