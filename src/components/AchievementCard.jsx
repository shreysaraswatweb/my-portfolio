    ...
    <GlassCard className="rounded-xl p-space-5">
      <div className="grid grid-cols-4 gap-space-2 text-center">
        {achievements.map((item) => {
          ...
          return (
            <div key={item.id} className="flex flex-col items-center gap-space-2">
              <IconChip className="h-space-10 w-space-10">
                <Icon className={['h-space-5 w-space-5', chipColors[item.icon]].join(' ')} strokeWidth={1.75} />
              </IconChip>
              <p className="text-body-lg font-bold text-text-primary">{item.value}</p>
